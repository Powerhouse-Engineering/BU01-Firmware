...
uint16_t startup_max_duty_cycle = 1400;
...
uint16_t low_rpm_level = 10; // thousand erpm used to set range for throttle resrictions
uint16_t high_rpm_level = 50; //
uint16_t throttle_max_at_low_rpm = 900;
...
void loadEEpromSettings()
{
    ...
    if (eepromBuffer.startup_power < 151 && eepromBuffer.startup_power > 49) {
        if(!eepromBuffer.comp_pwm){        // higher startup power for non-complementary pwm
            ...
            min_startup_duty = (eepromBuffer.startup_power * 10);
            minimum_duty_cycle = (eepromBuffer.startup_power * 10);
            stall_protect_minimum_duty = minimum_duty_cycle + 10;
			}
    } else {
